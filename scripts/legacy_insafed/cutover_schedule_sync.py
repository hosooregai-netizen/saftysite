...
def build_schedule_diff_rows(
    ...
    for legacy_site in legacy_sites:
        ...
        expected_rounds = {
            int(visit.get("round_no") or 0): visit
            for visit in legacy_site.get("visit_history", [])
        ...
        for round_no, live_row in live_rows.items():
            ...
            if is_legacy_selected(live_row, legacy_site_id):
                diff_rows.append(
                    {
                        "kind": "extra_legacy_selected_row",
                        "legacy_site_id": legacy_site_id,
                        "round_no": round_no,
                    }
                )
...
def build_memo_diff_rows(
    live_sites: list[dict[str, Any]],
    schedule_rows: list[dict[str, Any]],
) -> list[dict[str, Any]]:
    rows_by_site: dict[str, dict[int, dict[str, Any]]] = {}
    for row in schedule_rows:
...
def sync_site_schedule_rows(
    ...
    max_round = max(
        [int(legacy_site.get("total_rounds") or 0), *visit_by_round.keys(), *rows_by_round.keys()],
        default=0,
    )
    default_assignee = resolve_assignee(
        site,
        user_by_name,
        normalize_text(legacy_site.get("assigned_worker_name")),
    )
    ...
    for round_no in range(1, max_round + 1):
        ...
        if not current_row:
            audit_rows.append(
                {
                    "kind": "missing_round_after_generate",
                    "legacy_site_id": normalize_text(legacy_site.get("legacy_site_id")),
                    "round_no": round_no,
                }
            )
        ...
        if visit:
            assignee = resolve_assignee(
                site,
                user_by_name,
                normalize_text(visit.get("assigned_worker_name")),
            ) or default_assignee
...
def sync_site_memo_with_rows(client: Any, site: dict[str, Any], rows: list[dict[str, Any]]) -> None:
    ...
    next_envelope = {
        **envelope,
        "schedules": [serialize_schedule_row(row) for row in sorted(rows, key=schedule_round_no)],
    }
...
def is_legacy_selected(row: dict[str, Any], legacy_site_id: str) -> bool:
    return normalize_text(row.get("selection_reason_label")) == "Legacy InSEF import" or (
        f"legacy_site_id={legacy_site_id}" in normalize_text(row.get("selection_reason_memo"))
    )
...
def serialize_schedule_row(row: dict[str, Any]) -> dict[str, str | int]:
    return {
        ...
        "exceptionReasonCode": normalize_text(
            row.get("exception_reason_code") or row.get("exceptionReasonCode")
        ),
        ...
        "selectionConfirmedAt": normalize_text(
            row.get("selection_confirmed_at") or row.get("selectionConfirmedAt")
        ),
        "selectionConfirmedByName": normalize_text(
            row.get("selection_confirmed_by_name") or row.get("selectionConfirmedByName")
        ),
        "selectionConfirmedByUserId": normalize_text(
            row.get("selection_confirmed_by_user_id") or row.get("selectionConfirmedByUserId")
        ),
        "selectionReasonLabel": normalize_text(
            row.get("selection_reason_label") or row.get("selectionReasonLabel")
        ),
        "selectionReasonMemo": normalize_text(
            row.get("selection_reason_memo") or row.get("selectionReasonMemo")
        ),
