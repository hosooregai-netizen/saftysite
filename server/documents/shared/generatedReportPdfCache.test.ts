...
test('generated report pdf cache isolates uploaded assets by quality version', async () => {
  ...
  try {
    ...
    const previousVersion =
      'inspection-v10-template-text-layout-v2-measurement-unit-doc5-chart-hidpi-v6-total-12';
    const currentVersion = buildInspectionGeneratedPdfCacheVersion('12');

    assert.equal(INSPECTION_DOC5_CHART_PDF_CACHE_VERSION, 'doc5-chart-hidpi-v7');
    assert.equal(
      currentVersion,
