...
function buildHazardFallbackPayload(fileCount: number) {
...
function buildAgentFallbackPayload() {
  return {
...
async function analyzeHazardFiles(files: File[]): Promise<unknown> {
  const schema = {
    type: 'object',
...
async function analyzeAgentFile(file: File): Promise<unknown> {
  const schema = {
    type: 'object',
    additionalProperties: false,
