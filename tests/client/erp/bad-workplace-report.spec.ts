...
export async function runBadWorkplaceReportSmoke(config: ClientSmokePlaywrightConfig) {
  const harness = await createErpSmokeHarness('bad-workplace-report', config);
  ...
  try {
    ...
    await page.goto(`${harness.baseURL}/sites/site-1/bad-workplace/2026-03`, {
      waitUntil: 'load',
    });
    ...
    await page.getByRole('button', { name: '행 추가' }).waitFor({ state: 'visible' });
