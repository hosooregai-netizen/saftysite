...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Sync only targeted legacy sites into live admin schedules.")
    parser.add_argument("--export-root", required=True)
    parser.add_argument("--legacy-site-ids-path", required=True)
    ...
    parser.add_argument("--audit-dir", default=".artifacts/legacy-schedule-sync-targets")
    return parser
...
def main() -> int:
    ...
    target_ids = {
        normalize_text(line)
        for line in Path(args.legacy_site_ids_path).read_text().splitlines()
        if normalize_text(line)
    }
    audit_dir = ensure_dir(Path(args.audit_dir).resolve())
    scope = filter_cutover_bundle(load_export_bundle(export_root))
    target_sites = [
        site
        for site in scope["sites"]
        if normalize_text(site.get("legacy_site_id")) in target_ids
    ]
    ...
    for index, legacy_site in enumerate(target_sites, start=1):
        ...
        if index % 20 == 0:
            print({"processed": index, "total": len(target_sites), "last_legacy_site_id": legacy_site_id}, flush=True)

    out_path = audit_dir / "sync-results.jsonl"
