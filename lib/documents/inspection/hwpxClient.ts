...
const TEMPLATE_IMAGE_DONOR_URL = `/templates/inspection/${encodeURIComponent(
  INSPECTION_TEMPLATE_IMAGE_DONOR_FILENAME,
)}`;
...
function buildTemplateUrl(variant: InspectionTemplateVariant): string {
  return `/templates/inspection/${encodeURIComponent(getInspectionTemplateFilename(variant))}`;
}
...
const BASE_TEMPLATE_IMAGE_PLACEHOLDERS: TemplateImagePlaceholder[] = [
  ...
  },
  {
    ...
    row: 5,
    col: 1,
    placeholderPath: 'sec4.follow_ups[1].before_image',
    binaryItemId: 'tplimg30',
    repeatBlockPath: 'sec4.follow_ups',
  },
  {
    table: 3,
    row: 5,
    col: 4,
    placeholderPath: 'sec4.follow_ups[1].after_image',
    binaryItemId: 'tplimg31',
    repeatBlockPath: 'sec4.follow_ups',
  },
  {
    table: 3,
    row: 8,
    col: 1,
    placeholderPath: 'sec4.follow_ups[2].before_image',
    binaryItemId: 'tplimg32',
    repeatBlockPath: 'sec4.follow_ups',
  },
  {
    table: 3,
    row: 8,
    col: 4,
    placeholderPath: 'sec4.follow_ups[2].after_image',
    binaryItemId: 'tplimg33',
    repeatBlockPath: 'sec4.follow_ups',
  },
  {
    table: 5,
  ...
  if (variant === 'v10-1') {
    return [
      ...BASE_TEMPLATE_IMAGE_PLACEHOLDERS,
      {
        ...
        donorRow: 8,
        donorCol: 0,
        placeholderPath: 'sec10.accident_tracking.photo_image',
        binaryItemId: 'tplimg34',
      },
      {
        table: 7,
        row: 18,
        ...
        donorRow: 8,
        donorCol: 5,
        placeholderPath: 'sec10.accident_tracking.photo_image_2',
        binaryItemId: 'tplimg35',
      },
    ];
  }

  return BASE_TEMPLATE_IMAGE_PLACEHOLDERS;
}
...
function toCausativeLabel(key: string, measureLabelMap: Map<string, string>): string {
  ...
  const mapped =
    CAUSATIVE_AGENT_LABELS[normalized as keyof typeof CAUSATIVE_AGENT_LABELS] ??
...
function buildDoc5ChartImages(
  ...
  const cumulativeAccidentEntries: Doc5ChartEntry[] = hasStoredCumulativeEntries
...
function createEmptyActivity() {
  return { id: '', photoUrl: '', activityTitle: '', photoUrl2: '', activityType: '', content: '' };
}
...
function hasDocument12ActivityContent(item: ReturnType<typeof createEmptyActivity>): boolean {
  return Boolean(
...
function mapSessionToTemplateBinding(session: InspectionSession): TemplateBindingData {
  ...
  const activities = padFixedSlots(
    normalizeDocument12Activities(session.document12Activities)
      .filter((item) => hasDocument12ActivityContent(item))
      .slice(0, 2),
    2,
    createEmptyActivity,
  );
...
function expandRepeatBlocks(
  ...
  let currentXml = xml;
  const expandedImagePlaceholders = imagePlaceholders
    .filter((item) => !item.repeatBlockPath)
    .map((item) => ({ ...item }));
  ...
  for (const repeatBlockPath of REPEAT_BLOCKS) {
    ...
    const repeatImagePlaceholders = imagePlaceholders.filter(
...
function trimLeadingBlankParagraphPagesBeforeTable(sectionXml: string, tableIndex: number | null): string {
  ...
  if (!previousTable || !targetTable || previousTable.end >= targetTable.start) {
    return sectionXml;
  }

  const gapParagraphs = paragraphSpans(sectionXml)
    .filter((span) => span.start >= previousTable.end && span.end <= targetTable.start)
    .map((span) => ({
      ...span,
      xml: sectionXml.slice(span.start, span.end),
    }));

  if (
    gapParagraphs.length === 0
    || gapParagraphs.some((paragraph) => !isHwpxBlankParagraph(paragraph.xml))
  ) {
    return sectionXml;
  }

  const vertPositions = gapParagraphs.map((paragraph) => extractFirstLineSegMetric(paragraph.xml, 'vertpos'));
  if (vertPositions.some((value) => value == null)) {
    return sectionXml;
  }

  let trimParagraphCount = 0;
  for (let index = 0; index < vertPositions.length - 1; index += 1) {
    if ((vertPositions[index] ?? 0) > (vertPositions[index + 1] ?? 0)) {
      trimParagraphCount = index + 1;
    }
  }

  if (trimParagraphCount <= 0 || trimParagraphCount >= gapParagraphs.length) {
    return sectionXml;
  }

  return (
    sectionXml.slice(0, gapParagraphs[0].start) +
    sectionXml.slice(gapParagraphs[trimParagraphCount].start)
  );
}

export function normalizeInspectionFloatingTableGapPages(sectionXml: string): string {
  let normalizedXml = sectionXml;

  for (const markers of [DOC5_SECTION_TITLE_CANDIDATES, DOC7_SECTION_MARKERS] as const) {
    normalizedXml = trimLeadingBlankParagraphPagesBeforeTable(
      normalizedXml,
      findTableIndexByMarkers(normalizedXml, markers),
    );
  }
...
function ensureUniqueTableObjectIds(sectionXml: string): string {
...
function ensureCoverClientRepresentativePlaceholder(sectionXml: string): string {
  ...
  return sectionXml.replace(
    ...
    (_match, prefix: string, _currentValue: string, suffix: string) =>
      `${prefix}{cover.client_representative_name}${suffix}`,
  );
}
...
function applyVariantTemplateAnnotations(
  sectionXml: string,
  variant: InspectionTemplateVariant,
): string {
...
function restoreMissingTemplateImageSlots(
  ...
  for (const descriptor of imagePlaceholders) {
    if (descriptor.optional) {
      continue;
    }
...
async function loadTemplateBuffer(variant: InspectionTemplateVariant): Promise<ArrayBuffer> {
  const response = await fetch(`${buildTemplateUrl(variant)}?ts=${Date.now()}`, { cache: 'no-store' });
  if (!response.ok) {
...
export async function generateInspectionHwpxBlob(
  ...
  const templateImagePlaceholders = getTemplateImagePlaceholders(templateVariant);
  const binding = mapSessionToTemplateBinding(session);
  Object.assign(binding.images, buildDoc5ChartImages(session, _siteSessions));
  const templateBuffer = await loadTemplateBuffer(templateVariant);
  ...
  const sectionXmlWithVariantAnnotations = applyVariantTemplateAnnotations(
    sectionXmlWithTemplateOverlays,
    templateVariant,
  );
  const sectionXmlWithNormalizedFloatingTableGaps = normalizeInspectionFloatingTableGapPages(
    sectionXmlWithVariantAnnotations,
  );
  const sectionXmlWithVisitCountFooter = applyVisitCountPageFooter(
    sectionXmlWithNormalizedFloatingTableGaps,
  ...
  if (HWPX_GENERATION_MODE === 'template_native') {
    ...
    if (IMAGE_BINDING_MODE === 'text_only') {
      const expanded = expandRepeatBlocks(
        sectionXmlWithVisitCountFooter,
      ...
      const restored = restoreMissingTemplateImageSlots(
        sectionXmlWithVisitCountFooter,
        donorSectionXml,
        templateImagePlaceholders,
      );
