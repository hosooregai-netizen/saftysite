...
4. improvements는 현장 지적사항과 즉시 실행할 개선조치를 문장형 배열로 작성합니다.
5. improvements 각 문장은 "- " 로 시작합니다.
6. improvements 각 문장은 명사형 종결어미를 사용합니다.
7. laws는 위험 상황과 관련된 법적 근거를 최대 2건 작성합니다.
8. 정확한 조문을 확신할 수 없으면 법령명을 지어내지 말고 안전조치 원칙 수준으로만 작성합니다.
9. 사고유형 판단은 다음 후보를 우선 참고합니다: 추락, 떨어짐, 낙하, 맞음, 부딪힘, 충돌, 끼임, 깔림, 매몰, 감전, 화재·폭발, 화상, 붕괴, 전도, 찔림, 기타.
10. 비계, 작업발판, 단부·개구부, 사다리, 달비계, 고소작업대, 굴착기, 트럭, 이동식크레인 등 14개 기인물과 연결될 수 있는 경우 그 특성을 반영해 개선조치를 더 구체적으로 작성합니다.
11. 사진만으로 판단 불가한 정보는 빈 문자열 또는 보수적 추정으로 처리합니다.
12. JSON 외 텍스트는 절대 출력하지 마세요.
13. likelihood는 1~3 정수입니다.
14. severity는 1~3 정수입니다.
`.trim();
