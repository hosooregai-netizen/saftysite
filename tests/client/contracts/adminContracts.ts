...
export const ADMIN_FEATURE_CONTRACTS = {
  ...
  'admin-headquarters': {
    ...
    description:
      '건설사 목록이 safety-server list passthrough 기반 서버 페이지네이션, session cache, drilldown 진입, 생성/수정/삭제 흐름, K2B 엑셀 자동등록 진입, 저장 성공 후 empty-body 응답 fallback 재조회, 저장 timeout 시 모달 유지와 안내 메시지, deleted soft-delete hidden refresh를 유지한다.',
    routes: ['/admin?section=headquarters'],
    markers: ['건설사 목록', '건설사 추가', '엑셀로 추가', '건설사 수정', '현장 보기'],
    apis: ['GET /api/admin/headquarters/list', 'GET /api/admin/sites/list', 'POST /excel-imports/parse', 'POST /excel-imports/apply', 'POST /headquarters', 'PATCH /headquarters/:id', 'DELETE /headquarters/:id'],
    criticalActions: ['건설사 목록 페이지 로드', '건설사 생성', 'K2B 엑셀 자동등록 modal 진입', '건설사 수정', '건설사 수정 empty-body fallback 재조회 확인', '건설사 수정 timeout 시 모달 유지와 안내 메시지 확인', '현장 drilldown 진입', '건설사 삭제', 'drilldown 현장 생성/수정/삭제 직후 refresh 확인'],
  },
  'admin-users': {
    id: 'admin-users',
    description:
  ...
  'admin-schedules': {
    id: 'admin-schedules',
    description:
      '관제 일정 보드가 snapshot-backed calendar/queue와 lookups 기준으로 월간 캘린더, 월 네비게이션, queue, lookups, legacy import 선택 일정의 calendar 우선 노출, 관리자 읽기 전용 상세 확인 흐름을 유지한다.',
    routes: ['/admin?section=schedules&month=2026-04'],
    markers: ['일정/캘린더', '오늘', '방문 일정 상세'],
    apis: [
      'GET /api/admin/schedules/calendar',
      'GET /api/admin/schedules/queue',
      'GET /api/admin/schedules/lookups',
    ],
    criticalActions: ['관제 일정 월간 보드 조회', '월 네비게이션 이동', '확정 일정 칩 표시', 'legacy import 선택 일정 표시', '관제 일정 상세 modal 진입', '관제 일정 읽기 전용 확인'],
  },
  'admin-content': {
    id: 'admin-content',
    description:
