...
export const ADMIN_FEATURE_CONTRACTS = {
  'admin-control-center': {
    ...
    description:
      '관제 대시보드 overview/analytics가 admin 로그인 진입, overview-first 로딩, session cache 기반 재진입, KPI/차트/기준월 전환, 기간 전환, directory lookups 기반 계약 유형 옵션, site 계약 연동 매출, 과거 예정 일정을 실회차로 인정하는 집계, 직원별 표의 지연 중심 간소화, 현장별 표의 건설사/진행률 copy, export 진입 흐름, 그리고 자료 부족 표에서 현장/사업장 이름이 우선 읽히는 컬럼 배치를 유지한다.',
    ...
    criticalActions: ['admin 로그인 진입', 'overview 진입', 'analytics 진입', 'overview-first 초기 로딩 상태 유지', 'session cache 재진입 확인', '기간 전환', 'analytics 기준월 전환', 'analytics 계약유형 lookup 확인', 'site 계약 수정 반영 확인', 'K2B 회차 실적 집계 확인', '과거 예정 일정 실회차 반영 확인', '직원별 불필요 열 제거 확인', '현장별 건설사/진행률 표기 확인', '실행/남은 회차 KPI 확인', 'legacy 무일정 데이터 백필 확인', '엑셀 내보내기', '핵심 카드와 차트 확인'],
  },
  'admin-headquarters': {
  ...
  'admin-sites': {
    id: 'admin-sites',
    description:
      '현장 목록과 현장 메인이 safety-server list/lookups passthrough 기반 서버 페이지네이션, directory lookups 계약유형 옵션, planned 기본 생성, deleted soft-delete hidden, 등록 정보 편집, 저장 성공 후 empty-body 응답 fallback 재조회, 점검자 기준 배정, assignment 저장 전 전체 목록 재조회 없는 빠른 mutation 경로, K2B 회차 데이터 반영 동선을 유지한다.',
    routes: [
    ...
    criticalActions: ['현장 목록 페이지 이동', '현장 생성/수정', '현장 수정 empty-body fallback 재조회 확인', '계약유형 lookup option 확인', 'planned 기본 생성 확인', 'deleted 상태 hidden 확인', '현장 메인 진입', '현장 메인 quick edit', '지도요원 배정 modal 진입', '배정 mutation preflight 축소 확인', 'K2B 회차 데이터 반영'],
  },
  'admin-schedules': {
    id: 'admin-schedules',
    description:
