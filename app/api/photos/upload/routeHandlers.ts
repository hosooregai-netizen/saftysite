...
export interface PhotoAlbumUploadRouteDeps {
...
export async function handlePhotoAlbumUploadPost(
  request: Request,
  deps: PhotoAlbumUploadRouteDeps,
): Promise<Response> {
  try {
    ...
    const uploadedAsset = mapBackendPhotoAsset(
      await deps.uploadSafetyPhotoAssetServer(token, uploadFormData, request),
    );
