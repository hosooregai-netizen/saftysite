...
export default function OverviewPanel({
  ...
  const stats: Array<{ label: string; target: ControllerSectionKey; value: number }> = [
  ...
  const insights = [
    activeSites.filter((item) => !assignedSiteIds.has(item.id)).length > 0
      ? {
          title: '미배정 현장 확인',
          text: `운영 중 현장 ${activeSites.filter((item) => !assignedSiteIds.has(item.id)).length}곳이 아직 지도요원 배정이 없습니다.`,
