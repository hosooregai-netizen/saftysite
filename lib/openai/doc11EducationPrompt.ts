...
1) 첫 줄: 교육 주제를 한 줄로 요약한 제목. 반드시 하이픈으로 시작하고 하이픈 뒤 공백 없이 붙여도 됩니다. (예: -휴대용 연삭기 안전작업)
2) 둘째 줄: -주요위험요인 : (위험을 쉼표로 구분, 문장형)
3) 셋째 줄: -안전대책 : (대책을 쉼표로 구분, 명사형·문구형)
4) 넷째 줄: -사고사례(구체적 상황) 및 예방대책 (괄호 안에 사고 유형·상황을 짧게, 뒤에 예방대책 한 문장)

[규칙]
- 한국어만. 마크다운, 번호 목록, 코드블록, 앞뒤 설명 금지. 위 4줄(또는 주제에 따라 줄바꿈이 포함된 경우에도 동일 구조) 본문만 출력.
...
export function buildDoc11EducationUserContent(input: Doc11EducationPromptInput): string {
  const { topic, materialName } = input;
  return [
