...
export async function buildInspectionWordDocument(
  ...
  const documentXml = buildDocumentXml(buildInspectionDocumentBody(session, siteSessions, context));
  const zip = new JSZip();
  const contentTypes = `${CONTENT_TYPES_PREFIX}${buildContentTypeDefaults(context.assets)}${CONTENT_TYPES_SUFFIX}`;
  ...
  zip
    .folder('word')
    ?.folder('_rels')
    ?.file('document.xml.rels', buildDocumentRelationships(context.assets));
  ...
  zip
    ...
    ?.file(
      ...
      <cp:coreProperties xmlns:cp="http://schemas.openxmlformats.org/package/2006/metadata/core-properties" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:dcterms="http://purl.org/dc/terms/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
        <dc:title>건설재해예방 기술지도결과보고서</dc:title>
