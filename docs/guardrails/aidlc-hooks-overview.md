...
- smoke runner와 정본 smoke 문서 연결을 metadata에서 검증하고, 정적 생성 레지스트리로 drift를 막기
...
- contract metadata가 이제 smoke runner와 canonical smoke doc까지 소유하고, 생성된 `tests/client/smokeRegistry.generated.ts`가 실행 레이어를 고정한다.
- contract metadata와 smoke harness/runner 같은 guardrail config가 바뀌면 targeted 경로 대신 full validation 또는 scope-aware smoke로 승격한다.
...
- `generateSmokeRegistry.mjs --check` 기반 smoke metadata / generated registry drift 검사
...
- `tests/client/contracts/smoke-specs/**/*.md`
...
- `scripts/listSmokeRegistryContractIds.ts`
...
## metadata-driven smoke registry

이제 smoke-specific truth는 `featureContractMetadata.json`의 `smokeSpec` 아래에 모인다.
...
`scripts/generateSmokeRegistry.mjs`는 이 metadata를 읽어 [tests/client/smokeRegistry.generated.ts](/Users/mac_mini/Documents/GitHub/saftysite-real/tests/client/smokeRegistry.generated.ts:1) 를 생성한다.

생성 파일은 다음 두 레이어를 정적으로 export한다.

- `SMOKE_RUNNERS`
- `SMOKE_DOCS`

즉, `runSmoke.ts`는 이제 hand-maintained runner map을 직접 들고 있지 않고, 생성된 registry만 사용한다.

## smoke doc linkage

각 contract는 이제 정본 smoke 문서를 하나 가진다.

- canonical doc는 `tests/client/contracts/smoke-specs/**` 아래 markdown 파일이다.
- 추가 manual check나 proof 문서는 `relatedDocPaths`로 연결한다.
- validator는 canonical doc가 실제로 존재하고 `## Verification` 또는 `## Manual Check` 섹션을 갖는지 검사한다.

실패 메시지에는 canonical doc path를 같이 넣어서, “어느 markdown을 업데이트해야 하나?”를 바로 알 수 있게 했다.
...
guarded source가 없더라도 아래와 같은 파일이 바뀌면 `pre-push`는 관련 scope의 smoke set으로 승격한다.

- `tests/client/contracts/featureContractMetadata.json`
- `tests/client/contracts/*.ts`
- `tests/client/contracts/smoke-specs/admin/*.md`
- `tests/client/contracts/smoke-specs/erp/*.md`
- `tests/client/featureContracts.ts`
- `tests/client/smokeRegistry.generated.ts`
- `tests/client/fixtures/*SmokeHarness.ts`
...
예를 들면 admin harness만 바뀌면 admin smoke만, ERP runner가 바뀌면 ERP smoke와 `auth`만 실행하는 식이다.
...
| [scripts/generateSmokeRegistry.mjs](/Users/mac_mini/Documents/GitHub/saftysite-real/scripts/generateSmokeRegistry.mjs:1) | metadata에서 정적 smoke registry 생성 및 stale check |
| [scripts/smokeRegistrySupport.mjs](/Users/mac_mini/Documents/GitHub/saftysite-real/scripts/smokeRegistrySupport.mjs:1) | smoke runner/doc validation과 registry source 생성 |
| [tests/client/smokeRegistry.generated.ts](/Users/mac_mini/Documents/GitHub/saftysite-real/tests/client/smokeRegistry.generated.ts:1) | checked-in derived smoke runner/doc registry |
