...
The inspection session master-data path now requests only content types needed by the technical-guidance workspace, including `doc7_reference_material`. The DOC7 reference picker renders results in small batches with lazy thumbnail loading after the first visible items, and shows simple row numbers in the picker title only.
