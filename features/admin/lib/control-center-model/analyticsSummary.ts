...
export function buildSummaryCardsAndStats(
  ...
  const summaryCards = [
    ...
    { deltaLabel: comparisonWindow.changeLabel, deltaTone: getDeltaTone(comparisonWindow.previous ? calculateChangeRate(totalExecutedRounds, previousExecutedRounds) : null), deltaValue: formatDeltaValue(comparisonWindow.previous ? calculateChangeRate(totalExecutedRounds, previousExecutedRounds) : null), label: '실행 회차', meta: `${getCurrentWindowLabel(filters.period)} · 방문일 경과 기준`, value: `${totalExecutedRounds}회` },
    { deltaLabel: '현재 필터 기준', deltaTone: 'neutral' as const, deltaValue: '방문일 미도래', label: '남은 회차', meta: `${getCurrentWindowLabel(filters.period)} · 방문일 기준`, value: `${remainingRounds}회` },
  ];
