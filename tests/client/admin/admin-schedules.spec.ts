...
export async function runAdminSchedulesSmoke(config: ClientSmokePlaywrightConfig) {
  ...
  try {
    ...
    await page.getByRole('button', { name: '[김요원] 기존 현장' }).first().waitFor({
      state: 'visible',
    });
    ...
    await page.getByRole('button', { name: '[김요원] 기존 현장' }).first().click();
    ...
    if ((await scheduleDialog.getByText('이슈').count()) !== 0) {
      throw new Error('Expected existing schedule dialog to hide issue summary field.');
    }
